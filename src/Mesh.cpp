...
QVector<Triangle>
Mesh::create2dTessellationTriangles(const int tessellationLevel) {
  ...
  for (int x = 0; x < tessellationLevelX; x++) {
    for (int y = 0; y < tessellationLevelY; y++) {
      Vertex a(QVector3D(x / (float)tessellationLevelX,
                         y / (float)tessellationLevelY, 0));
      Vertex b(QVector3D((x + 1) / (float)tessellationLevelX,
                         y / (float)tessellationLevelY, 0));
      Vertex c(QVector3D(x / (float)tessellationLevelX,
                         (y + 1) / (float)tessellationLevelY, 0));
      Vertex d(QVector3D((x + 1) / (float)tessellationLevelX,
                         (y + 1) / (float)tessellationLevelY, 0));
