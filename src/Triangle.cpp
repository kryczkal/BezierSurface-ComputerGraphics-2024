...
void Triangle::draw(DrawData &drawData) {
  ...
  if (settings.triangleSettings.debugDraw &&
      drawData.textureOrColor.canConvert<QColor>()) {
      ...
      for (int xm = x - 5; xm <= x + 5; ++xm) {
        for (int ym = y - 5; ym <= y + 5; ++ym) {
          if (xm >= 0 && xm < width && ym >= 0 && ym < height) {
            ...
            drawData.canvas.setPixelColor(
                xm, ym, settings.triangleSettings.triangleVertexColor);
          }
        }
      }
    }
  ...
  static constexpr auto drawPixelDebug =
      [](DrawData &drawData, const Settings &settings, int y, int x, qreal w0,
         qreal w1, qreal w2) {
        float proximityCoef =
            settings.triangleSettings.triangleEdgeDrawProximityCoef;
        if (w0 < proximityCoef || w1 < proximityCoef || w2 < proximityCoef)
          drawData.canvas.setPixelColor(
              x, y, settings.triangleSettings.triangleEdgeColor);
        else
          drawData.canvas.setPixelColor(
              x, y, settings.triangleSettings.triangleFillColor);
      };
